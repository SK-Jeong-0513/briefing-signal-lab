...
- **★ 다음 세션 우선순위(최신):** ① 반도체 바스켓/드롭다운 티커 확정(`scripts/fetch_dashboard.py`의 BASKET·KRSEMI_OPTIONS) ② `public/assets/data/valuechain_manual.json` HBM 실수치·출처 교체 ③ 랜딩 `index.html` #dashboard 옛 6카드 티저를 dashboard.html로 연결/정리 ④ 기술·금융·경제 콘텐츠 실리서치본 교체 + 매주 발행 루틴(tech/finance/economy.js 이슈 + `mailer/Code.gs` CATS[].issues) ⑤ 유료 폼 관심분야 문항 + 로드맵 분야 개방 ⑥ (선택) 상관계수 배지·수출 일별 해상도.

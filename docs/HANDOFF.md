...
- **★★★ 주간 발행 파이프라인 완성·라이브(2026-07-16 세션):** 기술(ai-infra·semicon·power·space)+금융(kr-equity·us-equity·bond·commodity)+경제(macro) 전부 반자동 파이프 연결·프로덕션 검증. **로봇(월 06:00 KST cron)→`주간-초안` 시트 draft→사용자 approve→사이트 자동 표시(코드 수정 0).** 발행주 라벨="2026년 29주 (7월 13일)". 렌더러=헤드라이너 선택적+신규도메인+빈상태 "준비 중". robot DOMAINS=10, 사이트 live 분야=기술4·금융4·경제1. 상세=아래 "다음에 할 일" 3번. **잔여(사용자):** us-equity·bond·commodity·kr-equity·macro draft 선별 + `주간-초안` 시트 정리(검증차 반복 실행으로 W29 draft ~100행 누적 — approve 후 나머지 draft 삭제). bio·space·flows는 신호 쌓이면 개방.
- **방문자 수 측정 — 검토 완료, 구현 결정 대기(2026-07-16):** 정적 GitHub Pages라 백엔드 없음 → 클라이언트 비콘/외부 서비스 필요. 추천=**GoatCounter**(무료·쿠키없음·동의배너 불필요·스크립트 1줄) 또는 Cloudflare Web Analytics(무료·쿠키없음). 자가소유 대안=Apps Script 웹앱+`방문로그` 시트 비콘(스택 일치, 단 CORS는 no-cors/이미지 비콘·유니크 dedup은 localStorage·리포팅 직접). GA4는 쿠키 동의배너 필요라 보류 권장. **사용자가 방법 선택 시 구현**(각 페이지 </body> 앞 스크립트 삽입).

...
- **★ 다음 세션 우선순위(최신):** ~~① 반도체 바스켓/드롭다운 티커 확정~~ **완료(2026-07-08, 현재값 확정)** ~~② valuechain_manual.json HBM 실수치·출처 교체~~ **완료(2026-07-08)** ~~③ 랜딩 #dashboard 옛 6카드 티저 정리~~ **완료(2026-07-08)** ④ 기술·금융·경제 콘텐츠 실리서치본 교체 + 매주 발행 루틴(tech/finance/economy.js 이슈 + `mailer/Code.gs` CATS[].issues) ⑤ 유료 폼 관심분야 문항 + 로드맵 분야 개방 ⑥ (선택) 상관계수 배지·수출 일별 해상도.
...
9. **시장(주식) 탭 + AI 브리핑 워크플로 — 스펙 확정(2026-07-08 grill), [docs/market-plan.md](market-plan.md) 참조.**
   - **진척(2026-07-08): ①§6 린트 `scripts/lib/guard.py` 완료(검증) ②시장 탭 `market.html`+렌더러 라이브(구글 시트 CSV 연결: 시장-일일 gid=0/시장-종목 gid=2102188761, market.js에 URL) ③nav 전 페이지 재정리(기술·경제·금융·시장·대시보드·서재·경제캘린더, #samples 제거) 완료.** 렌더=캘린더 CSV 패턴 재사용, 출처는 http만 링크. **남은 것: (A) DEEPSEEK/GEMINI 키 Actions Secret + Sheet 쓰기 경로(Apps Script) → (B) IonQ 네이티브 수집기 `fetch_market.py`(§6 린트 호출) → (C) Telegram Sheet 파이프 → (D) 금융·경제 자동 교체 → (E) 기술 AI 초안 보조.** 시트 티커는 텍스트 서식으로(앞자리 0 보존: 005930). C 하이브리드(IonQ=Actions 네이티브 재구축, Telegram=서버 유지+Sheet 파이프). 무인 완전자동 + **§6 결정적 린트**(코드가 유일 방어선) + AI 라벨. 엔진=deepseek 주력/Gemini 폴백(engine.py 이식). 종목=Sheet 설정형(시드 반도체 바스켓). Google Sheet 워크북(시장-일일/시장-종목/종목설정) CSV 렌더. 범위 단계적(시장→금융·경제 샘플 교체→기술 초안 보조). Nav=기술·경제·금융·시장·대시보드·서재·경제캘린더. **선행 셋업(사용자): DEEPSEEK/GEMINI 키 Actions Secret + Google Sheet + 쓰기 경로(Apps Script 웹앱 추천).** 다음: 시장 탭 스캐폴드 + §6 린트 모듈부터.

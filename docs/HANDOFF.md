...
4. **무료 구독자 메일링(검토·결정 필요)** — 무료 구독자를 사이트로 유입시키는 활성화 채널로, 기술 브리핑 요약 + 사이트 링크를 이메일 발송. 사용자가 우선 구현 희망. 결정 필요: 발송 소스(구글폼 응답 시트), 발송 수단(Gmail SMTP=IonQ/Telegram 패턴, CLAUDE.md §13 부분거부 처리 준수), 개인정보 동의, 발송 주체(GitHub Actions 크론). Phase 1 정적 사이트라 발송은 Actions에서. **아웃바운드라 착수 전 확인.**

...
8. **서재(Library) 재구축 — 구현 스펙 확정(2026-07-08 grill), [docs/library-plan.md](library-plan.md) 참조.** nav "서재"를 랜딩 섹션→독립 `library.html`로 승격. `.md`+프런트매터 주기 업로드 리포트 아카이브(Action이 library.json 자동 생성, marked.js 클라이언트 렌더). type=report(메인)/note(운영자 서가). 전량 무료(미끼·SEO), 본문 KO. 카테고리 페이지에 "심층 리포트" 스트립. 초기 시드=루트 Archive/ 3개(우주SCM·CPO docx→md, code_artifact.svg→note). 다음 세션은 스펙대로 구현.

...
- 2026-07-20 당시 게이트: 승인은 사람 판단만 사용했다. 2026-07-27 확장에서는 화요일 마감 후 별도 검수 게이트를 모두 통과한 경우에만 조건부 자동 승인한다.
...
## 주간 발행 수명주기 확장 — 2026-07-27 확정

기존 “`approved` 즉시 사이트 표시” 규칙을 폐기한다. 행 승인과 호 발행을 분리하고 `BSL_market/주간-발행`을 관리자·공개 웹·메일러의 단일 원장으로 사용한다.

상태: `draft → approved → manual_ready|auto_ready → published|skipped`. 발행 후 늦은 승인분은 `rev.2+` 웹판 업데이트로 추가하고 이메일은 재발송하지 않는다.
...
관리자 콘솔은 발행 예약, 현재 원장 상태, 경고, 웹판 업데이트를 제공한다. 알림 채널은 운영자 이메일과 관리자 콘솔이다.
...
- 라이브 반영은 시트 열·탭 생성, CSV 게시, Actions Secret 등록, 두 Apps Script 재배포 후 별도 검증한다.

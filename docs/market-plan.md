...
## 일일 시황 항목 확장 (2026-07-30)

웹 일일 시황이 카테고리당 하루 1~2건뿐이던 문제. **콘텐츠가 없어서가 아니라** 파이프가 카테고리당 `제목 15자 + 한줄 90자` **하나만** 썼기 때문이다(시장 무관 시간대는 건너뜀 → 더 적어짐).

메일이 더 풍부했던 이유는 `시장-본문`(상세 브리핑)이 메일에만 실리기 때문이다. **사이트는 `시장-일일`·`시장-종목` CSV만 읽는다.**

**해결: 토픽을 `시장-일일` 행으로 확장.** 카테고리당 **최대 3건**(`MARKET_MAX_ITEMS`), 항목당 **2~3문장(180자)**.

- **`시장-본문`을 CSV로 게시하지 않는다.** 화면에서 `📰 채널별 요약`을 숨겨도 **원시 CSV에 남아** 채널 핸들이 유출된다. 그 채널 목록은 소싱 자산이다.
- 토픽은 `cat_summaries[cat]`(이미 카테고리별 원문 묶음)에서 뽑으므로 **분류가 자동으로 정해진다.** LLM에 카테고리를 판정시키지 않는다.
- **개수를 채우게 하지 않는다.** "최대 3개, 없으면 빈 배열". 억지로 채우면 마지막 항목이 잡음이 되고 주간 파이프의 반복 게재와 같은 실패가 된다.
- **§6 린트는 항목별.** 종전 `parse_signal`은 위반 1건에 카테고리 전체를 버렸다 → `parse_signals`가 위반 항목만 드롭.
- 시간대는 **제목 접두사 `[장전]`** 이 표시이자 사이트 정렬 키다. 별도 컬럼을 두지 않는다.

**메일 영향은 작다** — 일일 메일은 07:40 발송이라 **장전 행만** 담긴다(장중 13:00·마감 19:00은 그 뒤 기록). 웹은 하루치가 쌓이고 메일은 아침 분량(3건 → 최대 9건)만 는다.

**사이트(`90382fc`):** 정렬이 날짜만 비교해 같은 날짜는 시트 입력 순서(장전→장중→마감)가 남아 **하루 안에서 오래된 순으로 뒤집혀** 있었다. `mktDailySort`가 날짜 + 시간대 순위로 정렬하고, cap은 15→**45**(카테고리당 하루 최대 9행 기준 약 5일분).

**배포:** `deploy/deploy.ps1` 실행만. `$FILES`에 `notifier/sheets.py`가 이미 있다. **서버 재시작 불필요** — cron이 매번 `summarizer.py`를 새로 띄우므로 파일 교체로 충분하다(재시작이 필요한 건 FastAPI 대시보드뿐). 새 환경변수 없음. 되돌리기는 `notifier/sheets.py.bak2`(git 아님).

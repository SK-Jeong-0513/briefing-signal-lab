...
def draft_domain(domain, week):
    ...
    user = (
        ...
        "이 중 '선행 신호'(경제지 헤드라인보다 앞서 밸류체인 압력·병목·구조 변화를 시사하는 항목)가 될 만한 것을 "
        "최대 5개 고르고, 각각을 후보 신호 카드로 만드세요. 전문 요약이 아니라 후보만 만듭니다.\n"
        "규칙: 투자판단(매수·매도·목표가·비중) 표현 절대 금지. 밸류체인 기업은 '관찰'로만. 모르면 비워두세요.\n"
        "아래 JSON 배열로만 출력(설명·코드펜스 없이):\n"
        '[{"출처n": <헤드라인 번호>, "제목ko": "...", "제목en": "...", '
        '"한줄ko": "메커니즘/밸류체인 관찰 1줄", "한줄en": "...", '
        '"밸류체인": "관찰되는 밸류체인 후보(기업/공정)", "선행도": <1~5, 경제지보다 앞선 정도>}]\n'
        "선행 신호가 없으면 [] 만 출력.\n\n헤드라인:\n%s" % (domain["label"]["ko"], block)
    )

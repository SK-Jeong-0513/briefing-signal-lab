...
def main():
    ...
    # ⚠️ 전 도메인 0건은 반드시 빨간 run 으로 끝낸다.
    #
    # 2026-08-08 실행에서 LLM 키 3종이 모두 죽어 11개 도메인이 전부 '응답 없음'으로
    # 0건이 됐는데, 로그만 남기고 return 0 이라 워크플로가 초록으로 끝났다. 그래서
    # 그 주 사이트가 통째로 빈 것을 일주일 뒤에야 알았다. 주간은 주 1회 단발이라
    # 그 순간 실패하면 한 주가 통째로 빈다 — 일일(하루 3회)과 달리 메울 기회가 없다.
    #
    # 일부 도메인만 0건인 것은 실패로 보지 않는다. 어떤 주는 실제로 신호가 없을 수
    # 있고, 그것까지 빨간불로 만들면 경보가 무뎌져 진짜 사고를 놓친다.
    if not rows:
        print("[ERROR] 전 도메인 초안 0건 — LLM 키·할당량을 확인하세요. 시트에 쓰지 않고 실패로 끝냅니다.")
        return 1
    # 쓰기 실패도 빨간 run 으로 끝낸다 — 초록불이면 초안이 없는 걸 아무도 모른다.
